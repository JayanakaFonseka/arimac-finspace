...
export default function SolutionsDropdown({ items, onClose, style }: Props) {
  ...
  return (
    ...
    >
      {/* Left List */}
      <div className="w-[60%] h-fit grid grid-cols-1 md:grid-cols-2 pr-6">
        {items.map((item) => (
          <p
            key={item.name}
            onClick={() => setSelectedItem(item)}
            className={`flex w-full text-lg text-gray-700 font-normal p-4 mt-2 rounded-2xl cursor-pointer items-center max-w-[312px] max-h-[60px]
                        ${
                          selectedItem.name === item.name
                            ? "bg-[#E0E0FF] font-semibold pr-2"
                            : "hover:bg-[#EFEFFF]"
                        }`}
          >
            {item.name}
      ...
      <div className="relative w-[40%]">
        <Image
          ...
          width={578}
          height={287}
          className="w-full rounded-t-2xl h-[287px] object-cover"
        />
        {selectedItem.logo && (
